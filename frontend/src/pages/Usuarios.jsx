...
import {
    ...
    Search, ArrowUpRight, CheckCircle2, Clock, AlertCircle, LogOut,
    Edit3, Trash2, Plus, Save, Phone, Map, Loader2
} from "lucide-react";
...
const Usuarios = () => {
    ...
    const sincronizarEcosistema = useCallback(async () => {
        ...
        // 3. Carga aislada de Coincidencias IA
        try {
            const res = await fetch(`${GATEWAY_URL}/ms-coincidencias/api/coincidencias`);
            if (res.ok) {
                setCoincidencias(await res.json());
                gatewayResponde = true;
            }
        } catch (e) {
            console.warn("ms-coincidencias en periodo de warm-up.");
        }

        // 4. Carga aislada de Historial Clínico
        try {
        ...
        <div className="min-h-screen flex bg-[#FFFDF9] font-sans antialiased text-slate-800 flex-col md:flex-row relative">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <main className="p-8 flex-1">
                    ...
                    {activeTab === "notif" && <NotificacionesSection data={alertasHeader} />}
                </main>
            </div>
        </div>
    );
...
const CoincidenciasSection = ({ data, setData }) => {
    const handleValidar = async (id) => {
        try {
            const res = await fetch(`${GATEWAY_URL}/ms-coincidencias/api/coincidencias/${id}/validar`, { method: "POST" });
            if (res.ok) {
                setData(data.map(x => x.id === id ? { ...x, estado: "Confirmado" } : x));
                toast.success("Cruce biométrico aprobado.");
            }
        } catch (e) {
            toast.error("Falla de red en ms-coincidencias");
        }
    };
        ...
        <div className="space-y-4">
            {data.map((c) => (
                <div key={c.id} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <span className="text-[10px] font-mono font-bold text-slate-400">{c.id} · {c.estado}</span>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        {c.estado === "Pending" || c.estado === "Pendiente" && (
                            <button onClick={() => handleValidar(c.id)} className="bg-[#256944] text-white px-3 py-1.5 rounded-xl font-bold text-xs">Confirmar</button>
                        )}
                    </div>
                </div>
            ))}
...
/* NOTIFICACIONES */
const NotificacionesSection = ({ data }) => (
    <div className="space-y-3">
        {data.map((n, i) => (
            <div key={n.id || i} className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm flex items-start justify-between gap-4">
                <div>
                    <p className="text-xs font-bold text-slate-700">{n.origen || "RabbitMQ Broker"}</p>
                    <p className="text-xs text-slate-400 font-medium mt-0.5">{n.mensaje || n.msg}</p>
                </div>
            </div>
        ))}
    </div>
